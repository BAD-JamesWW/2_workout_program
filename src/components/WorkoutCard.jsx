...
export default function WorkoutCard(props){
    ...
    /** Or short circuit makes it so if trainingPlan object is undefined then we default
     * to an empty object and try to desctructure from that empty object which will also result in undefined
     * but is better because instead of breaking our code we still try to destructure from the empty object
    */
    ...
    return (
        ...
        <div className="workout-grid">
           ...
           <h6>Reps</h6>
           <h6 className="weight-input">Max Weight</h6>
           {workout.map((workoutExercise, indexOfWorkout) => {
            return (
                <React.Fragment key={indexOfWorkout}>
                    <div className="exercise-name">
                        <p>{indexOfWorkout + 1}. {workoutExercise.name}</p>
                        <button onClick={() => {        
                            setShowExerciseDescription({
                    ...
                    onChange={(eventWithValue) => {
                        handleAddWeight(workoutExercise.name, eventWithValue.target.value)
